...
# Package names
packages <- c("flexdashboard", "dplyr", "ggplot2", "plotly", "forcats", "htmlwidgets", "tidyr", "reactable", "reactablefmtr")
...
ggplotly(
  summary_dataset %>%
    ...
    ggtitle(
      paste(
        "Total minimal estimated audience of top 30 spenders since",
        format(start_date, "%d.%m.%Y")
      )
    ),
  tooltip = "text"
...
summary_dataset %>%
  ...
  reactable(
    ...
    columns = list(
      ...
      "total_min_reach" = colDef(name = "Total Audience", format = colFormat(separators = TRUE, digits = 0)),
      "per_ad_min_reach" = colDef(name = "Per Ad Min Audience", format = colFormat(separators = TRUE, digits = 0)),
...
ggplotly(
  regional_spenders %>%
    ...
    ungroup() %>% 
    mutate(regional_spend_1000 = round(regional_spend/1000, 0),
          page_name = reorder(page_name, regional_spend_1000)) %>% 
    ggplot(aes(
      x = region, y = regional_spend_1000, fill = page_name,
         text = paste(
          "Page:",
          page_name,
          "<br>Regional spend:",
          regional_spend_1000, "thousand CZK"
        )
    )) +
    geom_col(position = "dodge2") +  scale_x_discrete(
  labels = c(
    "HKK",
    "JHC",
    "JHM",
  ...
  labs(fill = "") +
  ggtitle(paste(
    "Top 10 spenders in each region (out of top 500 overall spenders) since",
    format(start_date, "%d.%m.%Y")
  )),
  tooltip = "text")
...
regional_spenders %>%
  ...
  setNames(gsub("avg_", "", names(.))) %>%
  select("page_name", "page_id","hkk", "jhc", "jhm", "kvk", "lbk", "msk", "olk", "pak", "pha", "plk", "stc", "ulk", "vys", "zlk") %>% 
  reactable(
    defaultPageSize = 15,
    theme = nytimes(),
    highlight = TRUE,
    ...
    filterable = TRUE,
    striped = TRUE,
    defaultColDef = colDef(filterable = FALSE, format = colFormat(prefix = "CZK ", separators = TRUE, digits = 0)),
    columns = list(
      "page_name" = colDef(name = "Page Name", sticky = "left", width = 120, format = colFormat()),
      "page_id" = colDef(name = "Page ID", sticky = "left", width = 120, format = colFormat())
  ))
```
