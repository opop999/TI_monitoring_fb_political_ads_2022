---
title: "Facebook ADS"
author: "Ondrej Pekacek for TI CZ"
  ...
  summary_dataset %>%
    ...
    ggplot(
      aes(
        ...
        y = reorder(page_name, avg_min_reach_million),
    ...
    scale_x_sqrt(
      breaks = seq(0, 5000, 25),
      labels = seq(0, 5000, 25)
    ) +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 4.5)) +
    ...
    ggtitle(
      paste(
        "Total minimal reach of Ads (square root scale) since",
...
spend_over_time <- ggplotly(
  time_dataset %>%
    ...
    mutate(
      ad_creation_time_week = as.Date(cut(ad_creation_time, breaks = "week", start.on.monday = TRUE)),
    ...
    geom_line() +
    geom_point(size = 0.8) +
    geom_vline(aes(xintercept = as.numeric(election_date)), color = "#563a3a", linetype = 2, size = 0.2) +
